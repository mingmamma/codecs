...
parent      = [K =  : Json2Codec, V =  : Json2Codec] extends Json2Codec[Map[K, V]] {
  def encode(value: Map[K, V]): Json2 =
    Json2Obj(
      value.map(
        (k: K, v: V) =>
          (summon[Json2Codec[K]].encode(k), summon[Json2Codec[V]].encode(v))
             match
            {
              case (Json2Str(kString), vJson2) =>
                kString -> vJson2
            }
      )
    )
  def decode(json: Json2): Map[K, V] = json mCURSOR null
} # -1,
parent span = <7147..7528>,
child       = def decode(json: Json2): Map[K, V] = json mCURSOR null # -1,
child span  = [7473..7477..7533]
...
case class Json2Obj(map: Map[String, Json2]) extends Json2
...
object Json2Codecs:
  ...
  given VectorCodec[A](using codecA: Json2Codec[A]): Json2Codec[Vector[A]] with
    ...
    def decode(json: Json2): Vector[A] = 
      json match
        case Json2Arr(elements) => elements.map(element => codecA.decode(element))

  given ArrayCodec[A: Json2Codec: ClassTag]: Json2Codec[Array[A]] with
    // the usage of summon SEEMS ODD but otherwise there would be difficulty
    def encode(value: Array[A]): Json2 = Json2Arr(value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
    def decode(json: Json2): Array[A] =
      json match
        case Json2Arr(elements) => elements.map(element => summon[Json2Codec[A]].decode(element)).toArray

  given ListCodec[A: Json2Codec]: Json2Codec[List[A]] with
    def encode(value: List[A]): Json2 = Json2Arr(value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
    def decode(json: Json2): List[A] =
      json match
        case Json2Arr(elements) => elements.map(element => summon[Json2Codec[A]].decode(element)).toList

  given SetCodec[A: Json2Codec]: Json2Codec[Set[A]] with
    def encode(value: Set[A]): Json2 = Json2Arr(value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
    def decode(json: Json2): Set[A] =
      json match
        case Json2Arr(elements) => elements.map(element => summon[Json2Codec[A]].decode(element)).toSet

  given MapCodec[K: Json2Codec, V: Json2Codec]: Json2Codec[Map[K, V]] with
    def encode(value: Map[K, V]): Json2 = 
      Json2Obj(
        value.map((k: K, v: V) =>
          (summon[Json2Codec[K]].encode(k), summon[Json2Codec[V]].encode(v)) match
            case (Json2Str(kString), vJson2) => kString -> vJson2  
          )
      )
    def decode(json: Json2): Map[K, V] =
      json m@@


  given OptionCodec[A: Json2Codec]: Json2Codec[Option[A]] with
    def encode(value: Option[A]): Json2 = 
      value match
        case Some(x) => summon[Json2Codec[A]].encode(x)
        case None => Json2Null
    def decode(json: Json2): Option[A] = 
      json match
        case Json2Null => None
        case _ => Some(summon[Json2Codec[A]].decode(json))

  given Tuple1Codec[A: Json2Codec]: Json2Codec[Tuple1[A]] with
    def encode(value: Tuple1[A]): Json2 = summon[Json2Codec[A]].encode(value._1)
    def decode(json: Json2): Tuple1[A] = Tuple1(summon[Json2Codec[A]].decode(json))

  given Tuple2Codec[A: Json2Codec, B: Json2Codec]: Json2Codec[Tuple2[A, B]] with
    def encode(value: Tuple2[A, B]): Json2 = 
      Json2Arr(Vector(summon[Json2Codec[A]].encode(value._1),
              summon[Json2Codec[B]].encode(value._2)))
    def decode(json: Json2): Tuple2[A, B] =
      json match
        // what machanism allow for this pattern match?
        // more idiomatic to just create a Tuple2 instance by (foo, bar) https://www.scala-lang.org/api/3.3.1/scala/Tuple2.html#
        case Json2Arr(Vector(a, b)) => (summon[Json2Codec[A]].decode(a), summon[Json2Codec[B]].decode(b))
