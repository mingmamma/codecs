...
parent      =  extends Json2Codec[Double] {
  def encode(value: Double) = Json2Num(value)
  def decode(json: Json2) = json mCURSOR null
} # -1,
parent span = <4866..5016>,
child       = def decode(json: Json2) = json mCURSOR null # -1,
child span  = [4971..4975..5881]
...
// trait primitiveCodes:
  ...
  given IntCodec: Json2Codec[Int] with
    ...
    def decode(json: Json2) = 
      json match
        case Json2Num(value) if value.isValidInt => value.toInt

  given DoubleCodec: Json2Codec[Double] with //= new Json2Codec[Double] {
    def encode(value: Double) = Json2Num(value)
    def decode(json: Json2) = 
      json m@@
  // }
