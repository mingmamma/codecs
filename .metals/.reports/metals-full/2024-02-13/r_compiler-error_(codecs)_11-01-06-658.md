...
parent      = [A =  : Json2Codec] extends Json2Codec[List[A]] {
  def encode(value: List[A]): Json2 =
    Json2Arr(
      value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
  def decode(json: Json2): List[A] = json mCURSOR null
} # -1,
parent span = <6388..6611>,
child       = def decode(json: Json2): List[A] = json mCURSOR null # -1,
child span  = [6556..6560..7534]
