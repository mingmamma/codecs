...
parent      = [A =  : Json2Codec] extends Json2Codec[Set[A]] {
  def encode(value: Set[A]): Json2 =
    Json2Arr(
      value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
  def decode(json: Json2): Set[A] = json mCURSOR null
} # -1,
parent span = <6719..6933>,
child       = def decode(json: Json2): Set[A] = json mCURSOR null # -1,
child span  = [6881..6885..7795]
