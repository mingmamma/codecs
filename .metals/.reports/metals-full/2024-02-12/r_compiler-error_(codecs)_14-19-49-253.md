...
parent      = [A =  : Json2Codec] extends Json2Codec[Array[A]] {
  def encode(value: Array[A]): Json2 =
    Json2Arr(
      value.map(element => summon[Json2Codec[A]].encode(element)).toVector)
  def decode(json: Json2): Array[A] = json mCURSOR null
} # -1,
parent span = <5915..6220>,
child       = def decode(json: Json2): Array[A] = json mCURSOR null # -1,
child span  = [6164..6168..7207]
